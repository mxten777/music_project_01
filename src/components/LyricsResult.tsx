...
export default function LyricsResult({ lyrics, chords }: Props) {
  ...
  return (
  <div style={{background: 'rgba(250,218,221,0.13)', backdropFilter: 'blur(8px)'}} className="mb-8 border-t pt-8 rounded-3xl shadow-2xl p-6 sm:p-10 transition-all duration-500 animate-fadein hover:shadow-pink-200/60 hover:scale-[1.018]" aria-live="polite" role="region" aria-labelledby="lyrics-result-heading">
  <h2 className="font-semibold mb-2 text-lg" style={{color: 'var(--color-japan-red)'}} id="lyrics-result-heading">🎵 변환 결과</h2>
  <div className="mb-2 text-sm" style={{color: 'var(--color-text-secondary)'}}>아래는 입력하신 시를 기반으로 생성된 가사와 코드입니다.</div>
  <pre className="p-3 rounded-xl whitespace-pre-wrap mb-2 border text-base shadow-inner bg-white/90 dark:bg-gray-900/80 border-pink-100/60 dark:border-pink-200/20" style={{color: 'var(--color-text)'}} aria-label="가사 결과">{lyrics}</pre>
